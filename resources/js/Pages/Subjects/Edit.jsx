...
export default function EditSubject({ subject }) {
    const { data, setData, put, processing, errors } = useForm({
        name: subject.name ?? '',
        name_khmer: subject.name_khmer ?? '',
        description: subject.description ?? '',
        grade_level: subject.grade_level ?? '',
        subject_type: subject.subject_type ?? 'core',
        credits: subject.credits ?? 1,
        hours_per_week: subject.hours_per_week ?? 1,
        fee: subject.fee ?? 0,
        ...
        <AuthenticatedLayout>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-end gap-3">
                    <Button type="button" variant="outline" onClick={() => router.get(`/subjects/${subject.id}`)}>Cancel</Button>
